...
class Transformer(nn.Module):
    ...
    def print_params_info(self, ):
        ...
        paramst = [p[1].numel() for p in self.named_parameters()]
        print ( paramsn, )
        print ( paramst )
...
def get_batch_real(split, gptconf):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(gptconf.data_dir, gptconf.dataset, 'train.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(gptconf.data_dir, gptconf.dataset, 'val.bin'), dtype=np.uint16, mode='r')
...
if __name__ == '__main__':
    ...
    #X = torch.randint(128256, (1, 128 ), dtype=torch.long).to('cuda')
    #logits = generator.model(X,0)
